...
def uploadData():
    ...
    if imgdata and name and age and gender and contact:
        ...
        imdb = db.Image( name= name, path=path, age=age, gender=gender, contact=contact, result=str(cat))
...
def viewData():
    ...
    if load_var('images.dk'):
        ...
        list_path =[]
        for i in content:
            li=str((i.name)+" "+str(i.age)+" "+(i.gender)+" "+str(i.contact)+" "+str(i.result))
            list_data.append(li)
            list_path.append(i.path)
        index=list_data.index(st.selectbox("select an image",list_data))
        select_img = list_path[index]
